...
mod tests {
    ...
    use crate::triomphe::{Arc, HeaderSlice};
    ...
    #[test]
    fn issue_13_empty() {
        crate::triomphe::Arc::from_header_and_iter((), iter::empty::<usize>());
    }

    #[test]
    fn issue_13_consumption() {
        let s: &[u8] = &[0u8; 255];
        crate::triomphe::Arc::from_header_and_iter((), s.iter().copied());
    }
